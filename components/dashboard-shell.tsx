...
const navItems = [
  {
    label: "Workspace",
    items: [
      { name: "Dashboard", icon: Home, color: "text-[#ef6f61]", active: true },
      { name: "AI Assistant", icon: Bot, color: "text-[#7c5cff]" },
      { name: "Calendar", icon: CalendarDays, color: "text-[#10a37f]" },
      { name: "Task / Kanban", icon: Grid3X3, color: "text-[#f59e0b]" },
    ],
  },
  {
    label: "Create",
    items: [
      { name: "Notes", icon: FileText, color: "text-[#0ea5e9]" },
      { name: "Whiteboard", icon: PenTool, color: "text-[#f97316]" },
      { name: "Pages / Spaces", icon: Layers3, color: "text-[#22c55e]" },
      { name: "AI Template Builder", icon: Gauge, color: "text-[#d946ef]" },
    ],
  },
  {
    label: "System",
    items: [{ name: "Settings", icon: Settings, color: "text-stone-500" }],
  },
]
                  ...
                  {group.items.map((item) => (
                    <button
                      ...
                      className={cn(
                        ...
                        item.active && "bg-[#ffe1dc] text-[#b84236] shadow-sm",
                        collapsed && "justify-center px-0"
                      )}
                    >
                      <item.icon className={cn("size-3.5 shrink-0", item.active ? "text-[#ef6f61]" : item.color)} />
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
            <div className="flex min-w-0 gap-3">
              <Button
                ...
                className="mt-1 size-9 shrink-0 text-[#625b53] lg:hidden"
              >
              ...
              <div className="mt-5 grid gap-3 md:grid-cols-3">
                {pulseItems.map((item) => (
                  <div
                    key={item.title}
                    className={cn(
                      "rounded-md border border-[#ded8ce] border-l-4 bg-[#fdfbf7] p-4",
                      item.border
                    )}
                  >
